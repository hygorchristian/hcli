...
const printCommand = (t, command, description) => {
  const space = '                         ';
  const distance = space.slice(0, -command.length);
...
const printHelper = async (toolbox) => {
  ...
  toolbox.print.success('Babbly Options:');
  printCommand(toolbox, 'babbly:sentry', 'Disable sentry dns url on Babbly project\n');
  printCommand(toolbox, 'babbly:sentry --enable', 'Enable sentry dns url on Babbly project\n');
